...
__INEQUALITY_REGEX = re.compile(r"^([-,\+]?\d{0,}x\d+)+([<,>])=(\d+)$")
...
def __valid_inequality(s: str) -> bool:
    return __INEQUALITY_REGEX.match(s) is not None
...
def input_inequality_matrix(prompt: str = "Inequation system: ") -> tuple[np.ndarray, np.ndarray]:
    # Get and validate input
    rows = []
    print(prompt)
    while True:
        try:
            ...
            if s == "":
                break
            # Check if valid
            if not __INEQUALITY_REGEX.match(s):
                print(f"Invalid inequation: '{s}'")
                continue
            # Store it
            rows.append(s)
        except KeyboardInterrupt:
